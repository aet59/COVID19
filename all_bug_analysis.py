...
def ans2part1(file_):
    ...
    all_closed_bugs = [x_ for x_ in all_closed_bugs if
                       'https://api.github.com/repos/RehanSaeed/Schema.NET/issues/' not in x_]
    all_closed_bugs = [x_ for x_ in all_closed_bugs if
                       'https://api.github.com/repos/vinitshahdeo/Water-Monitoring-System/issues/' not in x_]
    for cat in categs_bugs:
        ...
        cat_bug_ls = [x_ for x_ in cat_bug_ls if
                      'https://api.github.com/repos/vinitshahdeo/Water-Monitoring-System/issues/' not in x_]
        print('CATEG:{}, ALL:{}, CATEG_COUNT:{}, CATEG_PROP:{}'.format(cat, len(all_closed_bugs), len(cat_bug_ls), (
                    float(len(cat_bug_ls)) / float(len(all_closed_bugs))) * 100))
        print('*' * 50)
...
def ans2part2(repo_file, bug_file):
    ...
    for bug_categ in categs_bugs:
        for repo_cat in repo_categs:
            ...
            print('BUG_CATEG:{}, REPO_CATEG:{}, ALL_BUGS:{}, PER_CATEG_BUGS:{}, PROP:{}'.format(bug_categ, repo_cat,
                                                                                                len(per_repo_categ_bugs),
                                                                                                len(repo_categ_bug_categ),
                                                                                                float(
                                                                                                    len(repo_categ_bug_categ)) / float(
                                                                                                    len(per_repo_categ_bugs)) * 100))
            print('-' * 25)
        print('~' * 50)
...
def overallBugResolution(bug_fil):
    ...
    for cat in categs_bugs:
        ...
        print('CATEG:{}, MIN:{}, MEDIAN:{}, MAX:{} [DURATION:HOURS]'.format(cat, min(resolution_time),
                                                                            np.median(resolution_time),
                                                                            max(resolution_time)))
        print('*' * 50)
    ...
    for bug_categ in categs_bugs:
        for repo_cat in repo_categs:
            ...
            if len(resolution_time) > 0:
                print('REPO_CATEG:{}, BUG_CATEG:{}, MIN:{}, MEDIAN:{}, MAX:{} [DURATION:HOURS]'.format(repo_cat,
                                                                                                       bug_categ,
                                                                                                       min(resolution_time),
                                                                                                       np.median(
                                                                                                           resolution_time),
...
def durationRepoCateg(repo_file, bug_file):
    ...
    for repo_cat in repo_categs:
        ...
        print('REPO_CATEG:{}, MIN:{}, MEDIAN:{}, MAX:{} [DURATION:HOURS]'.format(repo_cat, min(resolution_time),
                                                                                 np.median(resolution_time),
                                                                                 max(resolution_time)))
        print('#' * 25)
    print(
        'ALL, MIN:{}, MEDIAN:{}, MAX:{} [DURATION:HOURS]'.format(min(mega_list), np.median(mega_list), max(mega_list)))
    print('#' * 25)
...
if __name__ == '__main__':
    ...
    print('FREQUENCY PER BUG CATEGORY')
    ans2part1(bug_categ_file)
    print('=' * 100)
    print('FREQUENCY PER REPO CATEGORY PER BUG CATEGORY')
    ans2part2(repo_categ_file, bug_categ_file)
    print('=' * 100)
    print('FREQUENCY PER REPO CATEGORY ')
